...
interface Stats {
  ...
  media: { total: number; draft: number };
}
...
const entityCards = [
  ...
  { key: 'media' as const, label: '图片库', icon: '🖼️', href: '/admin/media' },
];
  ...
  useEffect(() => {
    async function fetchStats() {
      try {
        ...
        const [b, p, a, c, e, co, t, m, bd, pd, ad, cd, ed, cod, td, md] = await Promise.all([
          ...
          fetch('/api/admin/media?pageSize=1', { headers }).then(r => r.json()),
          ...
          fetch('/api/admin/media?status=hidden&pageSize=1', { headers }).then(r => r.json()),
        ]);
        setStats({
          ...
          media: { total: m.total ?? 0, draft: md.total ?? 0 },
        });
      } catch {
        // ignore
      } finally {
