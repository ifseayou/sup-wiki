...
export const GET = withAdmin(async (request: NextRequest) => {
  try {
    ...
    const [countRows] = await pool.execute<RowDataPacket[]>(
      `SELECT COUNT(*) as total FROM sup_learn_articles ${where}`,
      params
    );
    const total = (countRows[0] as { total: number }).total;

    const [articles] = await pool.execute<RowDataPacket[]>(
      `SELECT * FROM sup_learn_articles ${where} ORDER BY CASE status WHEN 'published' THEN 0 ELSE 1 END, sort_order ASC, article_id ASC LIMIT ${pageSize} OFFSET ${offset}`,
      params
    );
    ...
    return NextResponse.json({ error: '获取学习文档列表失败' }, { status: 500 });
  }
});
...
export const POST = withAdmin(async (request: NextRequest) => {
  try {
    ...
    const { title, slug, category = 'muscle', summary, content, difficulty = 'beginner', sort_order = 0, status = 'draft' } = body;
    ...
    const [result] = await pool.execute<ResultSetHeader>(
      ...
      [title, slug, category, summary || null, content || null, difficulty, sort_order, status]
    );
    return NextResponse.json({ success: true, article_id: result.insertId }, { status: 201 });
  } catch (error) {
    console.error('创建学习文档失败:', error);
    return NextResponse.json({ error: '创建学习文档失败' }, { status: 500 });
  }
});
