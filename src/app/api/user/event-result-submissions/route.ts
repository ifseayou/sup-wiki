...
function getPdfFiles(formData: FormData) {
  const multiple = formData.getAll('files').filter((item): item is File => item instanceof File && item.size > 0);
  if (multiple.length > 0) return multiple;
  const single = formData.get('file');
  return single instanceof File && single.size > 0 ? [single] : [];
}

async function uploadPdfToOss(file: File, userId: number) {
  const safeName = file.name.replace(/[^\w.\-\u4e00-\u9fa5]+/g, '-').slice(0, 120);
  const ossKey = `sup-wiki/result-submissions/${userId}/${Date.now()}-${Math.random().toString(36).slice(2, 8)}-${safeName}`;
...
export async function GET(request: NextRequest) {
  ...
  try {
    const [rows] = await pool.execute<RowDataPacket[]>(
      `SELECT submission_id, event_id, batch_id, batch_file_index, batch_total, batch_label,
              event_name, event_date, location, original_filename,
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    for (let index = 0; index < files.length; index += 1) {
      ...
      const fileUrl = await uploadPdfToOss(file, user.user_id);
      const [result] = await pool.execute<ResultSetHeader>(
        `INSERT INTO sup_event_result_submissions (
           user_id, event_id, batch_id, batch_file_index, batch_total, batch_label,
           event_name, event_date, location, file_url,
      ...
      submissions.push(result.insertId);
    }
