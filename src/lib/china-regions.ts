...
export const CHINA_REGIONS: ProvinceOption[] = [
  { name: '北京市', cities: ['北京市'] },
  { name: '天津市', cities: ['天津市'] },
  { name: '上海市', cities: ['上海市'] },
  { name: '重庆市', cities: ['重庆市'] },
...
export function getProvinceOptions(current?: string | null) {
  const names = CHINA_REGIONS.map((item) => item.name);
  return current && !names.includes(current) ? [current, ...names] : names;
}

export function getCityOptions(province?: string | null, current?: string | null) {
  const region = CHINA_REGIONS.find((item) => item.name === province);
  const names = region?.cities || [];
  return current && !names.includes(current) ? [current, ...names] : names;
}
