...
function usage() {
  console.log('Usage: node scripts/import-qingtian-2024-results.js --input /private/tmp/qingtian-2024-results.json [--dry-run]');
}
...
function parseTimeToSeconds(input) {
  ...
  const dottedTime = raw.match(/^(\d+):(\d{2})\.(\d{1,3})$/);
...
async function resolveAthleteId(connection, item, athleteCache) {
  ...
  if (existingRows.length) {
    ...
    await connection.execute(
      ...
      [
        ...
        existingRows.length > 1 ? '青田重导发现同名候选，需后台确认' : '青田重导自动确认同名运动员',
      ]
    );
    athleteCache.set(key, athleteId);
    return athleteId;
  ...
  const [insertResult] = await connection.execute(
    `INSERT INTO sup_athletes (name, nationality, discipline, bio, status)
     VALUES (?, '中国', 'race', '由青田国际桨板公开赛成绩册导入自动生成的运动员草稿档案，待补充完整人物资料。', 'draft')`,
    [name]
  );
  ...
  await connection.execute(
    `INSERT IGNORE INTO sup_athlete_identity_links
      (athlete_id, normalized_name, display_name, gender_hint, team_hint, nationality_hint, confidence, status, note)
     VALUES (?, ?, ?, ?, ?, '中国', 0.85, 'confirmed', '青田重导自动创建草稿运动员')`,
    ...
    await connection.execute(
      `UPDATE sup_events
       SET name = ?, province = ?, city = ?, venue = ?, start_date = ?, end_date = ?,
           ...
           result_source_note = '本地成绩册重解析导入：青田国际桨板公开赛',
