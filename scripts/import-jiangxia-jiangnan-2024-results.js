...
function usage() {
  console.log('Usage: node scripts/import-jiangxia-jiangnan-2024-results.js --input /tmp/jiangxia-jiangnan-2024-results.json [--dry-run]');
}
...
async function findEventId(connection) {
  const [rows] = await connection.execute(
    `SELECT event_id, name, status
     FROM sup_events
     WHERE name = '2024桨下江南' OR name LIKE '%桨下江南%' OR slug LIKE '%jiangnan%'
     ORDER BY CASE WHEN status = 'published' THEN 0 ELSE 1 END, event_id ASC
     LIMIT 1`
  );
  if (!rows.length) throw new Error('未找到2024桨下江南赛事记录');
...
async function resolveAthleteId(connection, item, athleteCache) {
  ...
  if (existingRows.length) {
    ...
    await connection.execute(
      ...
      [
        ...
        existingRows.length > 1 ? '桨下江南重导发现同名候选，需后台确认' : '桨下江南重导自动确认同名运动员',
      ]
    );
    athleteCache.set(key, athleteId);
    return athleteId;
  ...
  const [inserted] = await connection.execute(
    `INSERT INTO sup_athletes (name, nationality, discipline, bio, status)
     VALUES (?, '中国', 'race', '由2024桨下江南18KM桨板项目成绩导入自动生成的运动员草稿档案，待补充完整人物资料。', 'draft')`,
    [name]
  );
  ...
  await connection.execute(
    `INSERT IGNORE INTO sup_athlete_identity_links
      (athlete_id, normalized_name, display_name, gender_hint, team_hint, nationality_hint, confidence, status, note)
     VALUES (?, ?, ?, ?, ?, '中国', 0.85, 'confirmed', '桨下江南重导自动创建草稿运动员')`,
...
async function updateEvent(connection, eventId, payload) {
  await connection.execute(
    `UPDATE sup_events
     SET result_status = 'extended_complete', result_source_note = ?, result_last_verified_at = NOW(), status = 'published'
     WHERE event_id = ?`,
    [payload.event.result_source_note, eventId]
  );
}
...
async function upsertSource(connection, eventId, payload) {
  ...
  const fileType = source.file_type === 'xlsx' ? 'excel' : source.file_type || 'excel';
  const [existing] = await connection.execute(
    `SELECT source_id FROM sup_event_result_sources
     WHERE original_path = ?
     ORDER BY CASE WHEN event_id = ? THEN 0 ELSE 1 END, source_id ASC
     LIMIT 1`,
    [source.original_path || null, eventId]
  );
  const params = [
    eventId,
    ...
    source.parser_name || 'parse-jiangxia-jiangnan-2024-results.py',
    'imported',
  ...
  if (existing.length) {
    ...
    await connection.execute(
      `UPDATE sup_event_result_sources
       SET event_id = ?, original_path = ?, file_name = ?, file_type = ?, source_url = ?, parser_name = ?,
...
function duplicateRankIssues(rows) {
  ...
  for (const row of rows) {
    const key = `${row.discipline}|${row.gender_group}|${row.board_class || ''}|${row.round_label || ''}|${row.rank_position}`;
