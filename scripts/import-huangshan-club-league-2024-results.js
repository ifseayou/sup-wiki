...
function usage() {
  console.log('Usage: node scripts/import-huangshan-club-league-2024-results.js --input .cache/huangshan-club-league-2024-results.json [--dry-run]');
}
...
function parseTimeToSeconds(input) {
  ...
  if (!raw || raw === '关门' || statusCode(raw)) return null;
  ...
  if (existing.length) {
    ...
    await connection.execute(
      ...
      [
        ...
        event.result_source_note || '黄山站成绩单导入',
        eventId,
      ]
    );
    return eventId;
...
async function upsertSources(connection, eventId, sources) {
  ...
  for (const source of sources) {
    ...
    if (existing.length) {
      ...
      sourceIds.set(source.source_key, sourceId);
      continue;
    }
    const [inserted] = await connection.execute(
    ...
    sourceIds.set(source.source_key, Number(inserted.insertId));
  }
  return sourceIds;
}
...
async function resolveAthleteId(connection, item, athleteCache) {
  ...
  if (existingRows.length) {
    ...
    await connection.execute(
      ...
      [
        ...
        existingRows.length > 1 ? '黄山站导入发现同名候选，需后台确认' : '黄山站导入自动确认同名运动员',
      ]
    );
    athleteCache.set(key, athleteId);
    return athleteId;
  ...
  const [inserted] = await connection.execute(
    `INSERT INTO sup_athletes (name, nationality, discipline, bio, status)
     VALUES (?, '中国', 'race', '由2024中国桨板俱乐部联赛黄山站成绩单导入自动生成的运动员草稿档案，待补充完整人物资料。', 'draft')`,
    [name]
  );
  ...
  await connection.execute(
    `INSERT IGNORE INTO sup_athlete_identity_links
      (athlete_id, normalized_name, display_name, gender_hint, team_hint, nationality_hint, confidence, status, note)
     VALUES (?, ?, ?, ?, ?, '中国', 0.85, 'confirmed', '黄山站导入自动创建草稿运动员')`,
...
async function insertResults(connection, eventId, payload, sourceIds, athleteCache, touchedAthletes) {
  const sourceByKey = new Map((payload.sources || []).map((source) => [source.source_key, source]));
  ...
  for (const group of chunk(payload.results, RESULT_BATCH_SIZE)) {
    ...
    for (const result of group) {
      ...
      const source = sourceByKey.get(result.source_key);
      const sourceId = sourceIds.get(result.source_key);
      if (!source || !sourceId) throw new Error(`missing source for result: ${result.source_key}`);
      ...
      values.push(
        ...
        typeof result.parse_confidence === 'number' ? result.parse_confidence : 0.98,
...
function summarize(payload) {
  ...
  for (const row of payload.results) {
    bySource.set(row.source_key, (bySource.get(row.source_key) || 0) + 1);
    const moduleKey = `${row.discipline} · ${row.gender_group} · ${row.round_label || '-'}`;
...
async function main() {
  ...
  if (!Array.isArray(payload.sources) || payload.sources.length !== 3) throw new Error('input must include 3 sources');
  ...
  for (const [key, count] of summary.bySource) console.log(`${String(count).padStart(4, ' ')}  source:${key}`);
  ...
  try {
    ...
    const sourceIds = await upsertSources(connection, eventId, payload.sources);
    await connection.execute(
    ...
    await insertResults(connection, eventId, payload, sourceIds, athleteCache, touchedAthletes);
    ...
    try {
      ...
      // Ignore rollback errors after a failed connection or completed rollback.
    }
    throw error;
  } finally {
