...
function usage() {
  console.log('Usage: node scripts/import-shizuishan-2025-results.js --input /tmp/shizuishan-2025-results.json [--dry-run]');
}
...
async function resolveAthleteId(connection, item, athleteCache) {
  ...
  if (existingRows.length) {
    ...
    await connection.execute(
      ...
      [
        ...
        existingRows.length > 1 ? '石嘴山站重导发现同名候选，需后台确认' : '石嘴山站重导自动确认同名运动员',
      ]
    );
    athleteCache.set(key, athleteId);
    return athleteId;
  ...
  const [inserted] = await connection.execute(
    `INSERT INTO sup_athletes (name, nationality, discipline, bio, status)
     VALUES (?, '中国', 'race', '由2025年中国桨板精英赛石嘴山站成绩册导入自动生成的运动员草稿档案，待补充完整人物资料。', 'draft')`,
    [name]
  );
  ...
  await connection.execute(
    `INSERT IGNORE INTO sup_athlete_identity_links
      (athlete_id, normalized_name, display_name, gender_hint, team_hint, nationality_hint, confidence, status, note)
     VALUES (?, ?, ?, ?, ?, '中国', 0.85, 'confirmed', '石嘴山站重导自动创建草稿运动员')`,
...
async function upsertSource(connection, eventId, payload) {
  ...
  const params = [
    ...
    source.parser_name || 'parse-shizuishan-2025-results.py',
    'imported',
