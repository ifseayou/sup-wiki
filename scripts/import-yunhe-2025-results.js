...
function usage() {
  console.log('Usage: node scripts/import-yunhe-2025-results.js --input /private/tmp/yunhe-2025-results.json [--dry-run]');
}
...
async function resolveAthleteId(connection, item, athleteCache) {
  ...
  if (existingRows.length) {
    ...
    await connection.execute(
      ...
      [
        ...
        existingRows.length > 1 ? '云和总决赛重导发现同名候选，需后台确认' : '云和总决赛重导自动确认同名运动员',
      ]
    );
    athleteCache.set(key, athleteId);
    return athleteId;
  ...
  const [insertResult] = await connection.execute(
    `INSERT INTO sup_athletes (name, nationality, discipline, bio, status)
     VALUES (?, '中国', 'race', '由云和总决赛成绩册导入自动生成的运动员草稿档案，待补充完整人物资料。', 'draft')`,
    [name]
  );
  ...
  await connection.execute(
    `INSERT IGNORE INTO sup_athlete_identity_links
      (athlete_id, normalized_name, display_name, gender_hint, team_hint, nationality_hint, confidence, status, note)
     VALUES (?, ?, ?, ?, ?, '中国', 0.85, 'confirmed', '云和总决赛重导自动创建草稿运动员')`,
...
async function main() {
  ...
  try {
    ...
    await connection.execute(
      `UPDATE sup_events
       SET name = ?, province = ?, city = ?, venue = ?, start_date = ?, end_date = ?,
           ...
           result_source_note = '本地成绩册第1-34页重解析导入',
    ...
    await connection.execute('UPDATE sup_event_result_sources SET event_id = NULL WHERE event_id = ? AND source_id <> ?', [EVENT_ID, sourceId]);
    ...
    console.log(`done event_id=${EVENT_ID} results=${payload.results.length} touchedAthletes=${ids.length}`);
  } catch (error) {
    try {
