...
function usage() {
  console.log('Usage: node scripts/import-wuxi-huishan-2026-results.js --input .cache/wuxi-huishan-2026-results.json [--dry-run]');
}
...
function parseTimeToSeconds(input) {
  ...
  return Number(hours) * 3600 + Number(minutes) * 60 + Number(seconds) + Number((millis || '0').padEnd(3, '0')) / 1000;
}
...
async function resolveAthleteId(connection, item, athleteCache) {
  ...
  if (existingRows.length) {
    ...
    await connection.execute(
      ...
      [
        ...
        existingRows.length > 1 ? '水运江苏无锡惠山导入发现同名候选，需后台确认' : '水运江苏无锡惠山导入自动确认同名运动员',
      ]
    );
    athleteCache.set(key, athleteId);
    return athleteId;
  ...
  const [inserted] = await connection.execute(
    `INSERT INTO sup_athletes (name, nationality, discipline, bio, status)
     VALUES (?, '中国', 'race', '由2026“水运江苏”桨板赛（无锡惠山站）成绩册导入自动生成的运动员草稿档案，待补充完整人物资料。', 'draft')`,
    [name]
  );
  ...
  await connection.execute(
    `INSERT IGNORE INTO sup_athlete_identity_links
      (athlete_id, normalized_name, display_name, gender_hint, team_hint, nationality_hint, confidence, status, note)
     VALUES (?, ?, ?, ?, ?, '中国', 0.85, 'confirmed', '水运江苏无锡惠山导入自动创建草稿运动员')`,
...
async function insertResults(connection, eventId, payload, sourceIdByKey, athleteCache, touchedAthletes) {
  ...
  for (const group of chunk(payload.results, RESULT_BATCH_SIZE)) {
    ...
    for (const result of group) {
      ...
      const source = sourceByKey.get(result.source_key) || payload.sources[0];
      const sourceId = sourceIdByKey.get(result.source_key) || sourceIdByKey.get(source.source_key);
      values.push(
        eventId,
        athleteId,
...
async function main() {
  ...
  if (!Array.isArray(payload.sources) || !payload.sources.length) throw new Error('input must include sources');
  ...
  try {
    ...
    for (const source of payload.sources) {
      sourceIdByKey.set(source.source_key, await upsertSource(connection, eventId, source));
    }
    await connection.execute(
    ...
    await insertResults(connection, eventId, payload, sourceIdByKey, athleteCache, touchedAthletes);
    ...
    if (args.dryRun) {
      ...
      console.log(JSON.stringify({ imported: payload.results.length, eventId, sources: sourceIdByKey.size, touchedAthletes: touchedAthletes.size }, null, 2));
    }
  } catch (error) {
    try {
      await connection.rollback();
