...
function usage() {
  console.log('Usage: node scripts/import-baitahu-shaoxing-bank-2025-results.js --input .cache/baitahu-shaoxing-bank-2025-results.json [--dry-run]');
}
  ...
  if (existing.length) {
    ...
    await connection.execute(
      ...
      [
        ...
        event.result_source_note || '白塔湖桨板成绩导入',
        eventId,
      ]
    );
    return eventId;
...
async function resolveAthleteId(connection, item, athleteCache) {
  ...
  if (existingRows.length) {
    ...
    await connection.execute(
      ...
      [
        ...
        existingRows.length > 1 ? '白塔湖导入发现同名候选，需后台确认' : '白塔湖导入自动确认同名运动员',
      ]
    );
    athleteCache.set(key, athleteId);
    return athleteId;
  ...
  const [inserted] = await connection.execute(
    `INSERT INTO sup_athletes (name, nationality, discipline, bio, status)
     VALUES (?, '中国', 'race', '由2025绍兴银行杯白塔湖湿地皮划艇桨板马拉松邀请赛成绩公示导入自动生成的运动员草稿档案，待补充完整人物资料。', 'draft')`,
    [name]
  );
  ...
  await connection.execute(
    `INSERT IGNORE INTO sup_athlete_identity_links
      (athlete_id, normalized_name, display_name, gender_hint, team_hint, nationality_hint, confidence, status, note)
     VALUES (?, ?, ?, ?, ?, '中国', 0.85, 'confirmed', '白塔湖导入自动创建草稿运动员')`,
