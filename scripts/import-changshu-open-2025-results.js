...
function usage() {
  console.log('Usage: node scripts/import-changshu-open-2025-results.js --input /private/tmp/changshu-open-2025-results.json [--dry-run]');
}
...
function parseTimeToSeconds(input) {
  ...
  const quote = raw.match(/^(\d{1,2})['′](\d{2})["”]?(\d{2})$/);
  const normalized = quote ? `${quote[1]}:${quote[2]}.${quote[3]}` : raw;
...
async function resolveAthleteId(connection, item, athleteCache) {
  ...
  if (existingRows.length) {
    ...
    await connection.execute(
      ...
      [
        ...
        existingRows.length > 1 ? '常熟站重导发现同名候选，需后台确认' : '常熟站重导自动确认同名运动员',
      ]
    );
    athleteCache.set(key, athleteId);
    return athleteId;
  ...
  const [insertResult] = await connection.execute(
    `INSERT INTO sup_athletes (name, nationality, discipline, bio, status)
     VALUES (?, '中国', 'race', '由2025中国桨板公开赛（常熟站）成绩册导入自动生成的运动员草稿档案，待补充完整人物资料。', 'draft')`,
    [name]
  );
  ...
  await connection.execute(
    `INSERT IGNORE INTO sup_athlete_identity_links
      (athlete_id, normalized_name, display_name, gender_hint, team_hint, nationality_hint, confidence, status, note)
     VALUES (?, ?, ?, ?, ?, '中国', 0.85, 'confirmed', '常熟站重导自动创建草稿运动员')`,
...
async function updateEvent(connection, payload) {
  ...
  await connection.execute(
    `UPDATE sup_events
     ...
     WHERE event_id = ?`,
    [
      event.name || '2025中国桨板公开赛（常熟站）',
      event.slug || 'china-sup-open-changshu-2025',
      event.province || '江苏省',
      event.city || '常熟市',
      event.start_date || '2025-06-28',
      event.end_date || event.start_date || '2025-06-29',
      '常熟站成绩册第29-127页重解析；积分汇总页未录入。',
      EVENT_ID,
    ]
  );
}
...
async function main() {
  ...
  for (const [key, count] of summarize(payload).slice(0, 80)) console.log(`${String(count).padStart(4, ' ')}  ${key}`);
