...
function usage() {
  console.log(`Usage:
  node scripts/import-local-race-results.js --input /path/results.json[|jsonl] [--dry-run] [--source-batch-size 10] [--submission-id 123] [--batch-id abc]
  node scripts/import-local-race-results.js --input /path/results.json[|jsonl] --event-id 123
...
function parseArgs(argv) {
  const args = { input: '', dryRun: false, sourceBatchSize: 10, resultBatchSize: 200, submissionId: null, batchId: '', eventId: null };
  for (let i = 2; i < argv.length; i += 1) {
    ...
    else if (item === '--submission-id') args.submissionId = Math.max(1, Number(argv[++i] || 0)) || null;
    else if (item === '--batch-id') args.batchId = String(argv[++i] || '').trim().slice(0, 40);
    else if (item === '--event-id') args.eventId = Math.max(1, Number(argv[++i] || 0)) || null;
...
async function requireEvent(connection, eventId) {
  const [rows] = await connection.execute(
    `SELECT event_id FROM sup_events WHERE event_id = ? LIMIT 1`,
    [eventId]
  );
  if (!rows.length) throw new Error(`event_id ${eventId} not found`);
  return Number(rows[0].event_id);
}

async function upsertSource(connection, eventId, source, resultsLength) {
  const resultSubmissionId = Number(source.result_submission_id || 0) > 0 ? Number(source.result_submission_id) : null;
  const resultSubmissionBatchId = String(source.result_submission_batch_id || '').trim().slice(0, 40) || null;
  const metadata = {
    ...(source.metadata || {}),
    source_kind: source.source_kind || source.metadata?.source_kind || 'local_result_book',
    result_submission_id: resultSubmissionId,
    result_submission_batch_id: resultSubmissionBatchId,
  };
  const [existing] = await connection.execute(
...
async function markResultSubmissionImported(connection, source, resultsLength) {
  if (!resultsLength) return;
  const submissionId = Number(source.result_submission_id || 0) > 0 ? Number(source.result_submission_id) : null;
  const batchId = String(source.result_submission_batch_id || '').trim().slice(0, 40);
  const note = `导入完成：${source.file_name || '成绩册'}，${resultsLength} 条成绩`;
  if (submissionId) {
    await connection.execute(
  ...
  if (batchId) {
    await connection.execute(
      ...
      [note, batchId, source.source_url || '', source.file_name || '']
    );
  }
}
...
async function importPayload(connection, payload, args, athleteCache) {
  const sourceInput = {
    ...(payload.source || {}),
    result_submission_id: payload.source?.result_submission_id || args.submissionId || null,
    result_submission_batch_id: payload.source?.result_submission_batch_id || args.batchId || null,
  };
  ...
  if (!args.dryRun && results.length && isDistinctiveFileName(sourceInput.file_name)) {
    const [existingSources] = await connection.execute(
      `SELECT source_id FROM sup_event_result_sources
       WHERE file_name = ? AND imported_rows > 0 AND COALESCE(original_path, '') <> COALESCE(?, '')
       LIMIT 1`,
      [sourceInput.file_name, sourceInput.original_path || '']
    );
  ...
  try {
    const eventId = args.eventId ? await requireEvent(connection, args.eventId) : await upsertEvent(connection, payload.event || {});
    const sourceId = await upsertSource(connection, eventId, sourceInput, results.length);
    const touchedAthletes = new Set(await insertResultRowsBatch(connection, eventId, sourceId, sourceInput, results, athleteCache, args.resultBatchSize));
    await markResultSubmissionImported(connection, sourceInput, results.length);
