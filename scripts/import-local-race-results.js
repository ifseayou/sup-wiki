...
function usage() {
  console.log(`Usage:
  node scripts/import-local-race-results.js --input /path/results.json[|jsonl] [--dry-run]
...
async function resolveAthleteId(connection, result) {
  ...
  if (identityRows.length) return Number(identityRows[0].athlete_id);
...
async function syncAthleteRaceTimes(connection, athleteId) {
  const [rows] = await connection.execute(
    `SELECT er.discipline, er.round_label, er.result_label, er.finish_time, e.start_date, e.event_id, e.name AS event_name
...
async function insertResultRow(connection, eventId, sourceId, source, result) {
...
async function importPayload(connection, payload, dryRun) {
  ...
  try {
    ...
    for (const result of results) {
      const athleteId = await insertResultRow(connection, eventId, sourceId, payload.source || {}, result);
    ...
    for (const athleteId of touchedAthletes) await syncAthleteRaceTimes(connection, athleteId);
    await connection.commit();
    return { eventId, sourceId, results: results.length, athletes: touchedAthletes.size };
  } catch (error) {
...
async function main() {
  ...
  try {
    for (const payload of payloads) {
      const result = await importPayload(connection, payload, args.dryRun);
      summary.sources += 1;
