...
function usage() {
  console.log('Usage: node scripts/import-wuxi-club-league-2025-results.js --input /private/tmp/wuxi-club-league-2025-results.json [--dry-run]');
}
...
async function resolveAthleteId(connection, item, athleteCache) {
  ...
  if (existingRows.length) {
    ...
    await connection.execute(
      ...
      [
        ...
        existingRows.length > 1 ? '无锡站重导发现同名候选，需后台确认' : '无锡站重导自动确认同名运动员',
      ]
    );
    athleteCache.set(key, athleteId);
    return athleteId;
  ...
  const [inserted] = await connection.execute(
    `INSERT INTO sup_athletes (name, nationality, discipline, bio, status)
     VALUES (?, '中国', 'race', '由2025中国桨板俱乐部联赛无锡站成绩册导入自动生成的运动员草稿档案，待补充完整人物资料。', 'draft')`,
    [name]
  );
  ...
  await connection.execute(
    `INSERT IGNORE INTO sup_athlete_identity_links
      (athlete_id, normalized_name, display_name, gender_hint, team_hint, nationality_hint, confidence, status, note)
     VALUES (?, ?, ?, ?, ?, '中国', 0.85, 'confirmed', '无锡站重导自动创建草稿运动员')`,
...
async function updateEvent(connection, payload) {
  ...
  await connection.execute(
    `UPDATE sup_events
     ...
     WHERE event_id = ?`,
    [
      event.name || '中国桨板俱乐部联赛无锡站',
      event.slug || 'china-sup-club-league-wuxi-2025',
      event.province || '江苏省',
      event.city || '无锡市',
      event.start_date || '2025-07-07',
      event.end_date || '2025-07-07',
      '无锡站扫描版成绩册第20-50页本地OCR重解析；第2-19页积分未录入。',
      EVENT_ID,
    ]
  );
}
...
async function insertResults(connection, items, sourceId, payload, athleteCache, touchedAthletes) {
  const insertedRows = [];
  ...
  for (const group of chunk(items, RESULT_BATCH_SIZE)) {
    ...
    for (const result of group) {
      ...
      values.push(
        ...
        typeof result.parse_confidence === 'number' ? result.parse_confidence : 0.9,
...
async function main() {
  ...
  console.log(`Team result rows: ${payload.results.filter((row) => Array.isArray(row.team_members) && row.team_members.length).length}`);
  ...
  try {
    ...
    const insertedRows = await insertResults(connection, payload.results, sourceId, payload, athleteCache, touchedAthletes);
